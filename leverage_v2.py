...
def simulate_price_path(initial_price, drift, volatility, days, dt=1 / 365):
    ...
    for t in range(1, steps + 1):
        prices[t] = prices[t - 1] * np.exp(
            ...
            + daily_volatility * np.sqrt(dt) * Z[t - 1]
        )
...
def simulate_leveraged_position(
    initial_deposit, price_path, liquidation_threshold, safety_margin=0.1
):
    """
    ...
    for day, price in enumerate(price_path):
        ...
        if total_borrowed > 0:
            ...
            health_factor = (
                collateral_value * liquidation_threshold / total_borrowed_value
            )
        else:
...
def run_monte_carlo_simulation(
    ...
    results = {
        ...
        "avg_days_to_liquidation": (
            np.mean(liquidation_days) if liquidation_days else None
        ),
        "median_days_to_liquidation": (
            np.median(liquidation_days) if liquidation_days else None
        ),
        "avg_survived_leverage": (
            np.mean(survived_max_leverages) if survived_max_leverages else None
        ),
        "avg_liquidated_leverage": (
            np.mean(liquidated_max_leverages) if liquidated_max_leverages else None
        ),
...
def plot_simulation_results(results):
    ...
    unique_leverages = sorted(
        list(set([round(x, 1) for x in results["max_leverages"]]))
    )
    avg_returns = []
    ...
    for lev in unique_leverages:
        ...
        indices = [
            i for i, x in enumerate(results["max_leverages"]) if round(x, 1) == lev
        ]

        # Calculate returns for these positions
        returns = [
            (
                (results["price_path"][-1] / results["price_path"][0] - 1) * lev
                if i not in results["liquidation_days"]
                else -1
            )
            for i in indices
        ]
    ...
    axes[0, 0].plot(
        optimal_leverage, max_return, "ro", label=f"Ótimo: {optimal_leverage:.1f}x"
    )
    axes[0, 0].legend()
    ...
    if results["liquidation_days"]:
        ...
        axes[0, 1].axvline(
            results["avg_days_to_liquidation"],
            color="blue",
            linestyle="--",
    ...
    axes[1, 0].pie(
        [results["liquidation_rate"], survival_rate],
        labels=[
            f'Liquidado ({results["liquidation_rate"]:.1f}%)',
...
if __name__ == "__main__":
    ...
    if results["avg_days_to_liquidation"] is not None:
        print(
            f"Average days to liquidation: {results['avg_days_to_liquidation']:.1f} days"
        )
    else:
    ...
    if results["avg_survived_leverage"] is not None:
        print(
            f"Average leverage (survived positions): {results['avg_survived_leverage']:.2f}x"
        )

    if results["avg_liquidated_leverage"] is not None:
        print(
            f"Average leverage (liquidated positions): {results['avg_liquidated_leverage']:.2f}x"
        )
